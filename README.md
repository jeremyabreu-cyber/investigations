# Incident Response Investigations

## Overview
This repository documents hands-on incident response investigation workflows practiced in a home lab environment. The focus is on structured analysis, log correlation, and decision-making during security investigations rather than tooling alone.

The investigations documented here are designed to reflect real-world SOC and incident response scenarios.

## Scope
- Endpoint and authentication-based investigations
- SIEM-driven analysis and timeline reconstruction
- Behavioral analysis of suspicious activity
- Documentation of investigative thought processes

## Investigation Structure
Each investigation includes:
- Initial indicator or alert
- Data sources reviewed
- Investigation steps and pivots
- Findings and assessment
- Outcome and lessons learned

## Purpose
The goal of this repository is to demonstrate practical incident response thinking, investigative methodology, and clear documentation that supports containment, remediation, and detection improvement.
